...
pu = pandas.json_normalize(alert_names) #put json inside a dataframe
pu['createdOn']=(pandas.to_datetime(pu['createdOn'],unit='ms')).apply(lambda x: x.tz_localize('UTC').tz_convert('America/Chicago'))
...
pu.to_csv('alert_names_{}.csv'.format(now), sep=',', encoding='utf-8', index=False, date_format='%m-%d-%y || %I:%M:%S %p CDT%z') 
print('Done.')
