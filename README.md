...
Just wanted to contribute something back to the community, this repo is a result of work I've done as part of an engagement with Palo Alto and Google as a Security Architect. 46 additional scripts: import scripts, alert scripts and backup scripts serve as the base. This fork leverages 2 Python files from the base project (pc_lib_general.py & pc_lib_api.py which has been updated for more API calls), is more GCP focused (plus AWS for alerts and compliance report) and requires installation of the popular Pandas open source library. Why Pandas instead of just using something native like CSVwriter to accomplish some of the tasks? If you are unfamiliar with the data analysis/manipulation flexibility provided the Pandas project which has over 27,000 stars, 11,000 forks, and 1,000 watchers (as of December 2020), here's a brief intro from the creators:
